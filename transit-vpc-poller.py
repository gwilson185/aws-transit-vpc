######################################################################################################################
# Customized to support VRFs/Routing Domains by George Wilson
    ...
    if json.dumps(rd['Count']) == '1':
        if 'vrf_asn' in json.dumps(rd['Items']):
            ...
            s3.put_object(
                ...
                Bucket=bucket_name,
                Key=bucket_prefix + 'CSR1/' + 'newroute.conf',
...
def updateConfigXML(xml, config, vgwTags, account_id, csr_number,routedomain):
    ...
    # ConfigureRouteDomain
    newXml = xmldoc.createElement("route_domain")
    newXml.appendChild(xmldoc.createTextNode(routedomain['rd_name']))
    transitConfig.appendChild(newXml)
